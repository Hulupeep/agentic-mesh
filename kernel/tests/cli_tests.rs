        ...
        nodes: vec![
            Node {
                ...
                args: Some(HashMap::from([
                    ("q".to_string(), json!("initial query")),
                ])),
                bind: None,
                out: Some(HashMap::from([("query_result".to_string(), "result".to_string())])),
            }
        ],
        edges: None,
...
async fn test_context_variable_resolution() {
    ...
    ctx.variables.insert("query_var".to_string(), json!("test query"));
    ctx.variables.insert("limit_var".to_string(), json!(10));
    
    // Create args with variable references
    let args_with_refs = HashMap::from([
        ("q".to_string(), "$query_var".to_string()),
        ("limit".to_string(), "$limit_var".to_string()),
        ("literal".to_string(), "not_a_variable".to_string()),
    ]);
    ...
    if let Some(serde_json::Value::Object(resolved_map)) = resolved {
        ...
        assert_eq!(resolved_map.get("q").unwrap().as_str().unwrap(), "test query");
        assert_eq!(resolved_map.get("limit").unwrap().as_i64().unwrap(), 10);
        assert_eq!(resolved_map.get("literal").unwrap().as_str().unwrap(), "not_a_variable");
    } else {
        panic!("Resolved args should be an object");
    }
    
    // Test with non-existent variable (should use literal)
    let args_with_bad_ref = HashMap::from([
        ("missing".to_string(), "$nonexistent_var".to_string()),
    ]);
    ...
    if let Some(serde_json::Value::Object(resolved_map)) = resolved_bad {
        ...
        assert_eq!(resolved_map.get("missing").unwrap().as_str().unwrap(), "$nonexistent_var");
    } else {
        panic!("Resolved args should be an object");
    }
