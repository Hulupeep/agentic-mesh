...
fn test_trace_signing() {
    ...
    // Sign the trace
    signer
        .sign_trace(&mut trace)
        .expect("Should sign trace successfully");

    // Verify the signature
    let is_valid = trace
        .verify_signature(signer.get_public_key())
...
fn test_trace_signature_verification_failure() {
    ...
    let verification_result = trace.verify_signature(signer2.get_public_key());
