...
impl Trace {
    ...
    pub fn with_plan_id(plan_id: String, event_type: String, step_id: String, description: String) -> Self {
        Self {
            plan_id,
            step_id,
            ts: Utc::now(),
    ...
    pub fn sign(&mut self, keypair: &Keypair) -> Result<(), TraceError> {
        let message = format!("{}:{}:{}:{}", self.plan_id, self.step_id, self.ts, self.event_type);
        let signature: Signature = keypair.sign(message.as_bytes());
        self.signature = Some(base64::encode(signature.to_bytes()));
        Ok(())
    }

    pub fn verify_signature(&self, public_key: &ed25519_dalek::PublicKey) -> Result<bool, TraceError> {
        if let Some(ref sig_str) = self.signature {
            let signature_bytes = base64::decode(sig_str)
                .map_err(|e| TraceError::SignatureError(e.to_string()))?;
            
            if signature_bytes.len() != 64 {
                return Err(TraceError::SignatureError("Invalid signature length".to_string()));
            }
            ...
            Err(TraceError::SignatureError("No signature present".to_string()))
        }
    }
}
...
mod tests {
    ...
    fn test_trace_signature() {
        ...
        let mut trace = Trace::new("test_event".to_string(), "step_1".to_string(), "test trace".to_string());
