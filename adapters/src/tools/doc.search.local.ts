...
async function initializeDB(): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    db = new sqlite3.Database('./search_index.db', (err) => {
      if (err) {
        reject(err);
        return;
      }
...
export const docSearchLocal: ToolHandler = {
  spec: {
  ...
  invoke: async (args: SearchArgs): Promise<any> => {
    if (!db) {
      await initializeDB();
    }
    ...
    const hits = results.map((row: any) => ({
      id: row.id,
      uri: row.uri,
      score: row.score,
      snippet: row.snippet,
