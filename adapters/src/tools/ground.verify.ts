/**
 * ground.verify - Toy evidence verifier
 */
...
function wordFreq(text: string, word: string): number {
  const words = text.toLowerCase().split(/\W+/);
  return words.filter(w => w === word.toLowerCase()).length;
}

// Helper function to calculate BM25 score
function calculateBM25Score(query: string, document: string, avgDocLength: number): number {
...
export const groundVerify: ToolHandler = {
  spec: {
    name: 'ground.verify',
  ...
  invoke: async (args: VerifyArgs): Promise<any> => {
    ...
    for (let i = 0; i < claims.length; i++) {
      ...
      let bestSupport: { source: Hit, confidence: number, explanation: string } | null = null;
      let bestContradiction: { source: Hit, confidence: number, explanation: string } | null = null;
